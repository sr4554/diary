...
def derandomise():
    # Use raw string for file path to avoid backslash issues
    file = open(r"C:\Users\sange\OneDrive\Desktop\travel project\Diary-app-main\dbdiary.txt", "r")
    contents = file.read()
...
def reader():
    ...
    if d in contents:
        ans = contents[
            contents.index(d) + len(str(d)) + 8 : contents.index("#end", contents.index(d) + 1)
        ]
...
def writer():
    ...
    with open(r"C:\Users\sange\OneDrive\Desktop\travel project\Diary-app-main\dbdiary.txt", "w") as f:
        f.write(randomise(finalised_product))  # Write the entire randomized string
